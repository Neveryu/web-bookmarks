...
3、如果要在报错的时候终止 `js` 文件的生成，可以在 `tsconfig.json` 中配置 `noEmitOnError` 即可。 关于 `tsconfig.json`，请参阅官方手册（中文版）。 https://zhongsp.gitbooks.io/typescript-handbook/content/doc/handbook/tsconfig.json.html
...
【类实现接口】实现（implements）是面向对象中的一个重要概念
